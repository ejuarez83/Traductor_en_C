    	...
    	//cout << "Escriba el texto que desea agregar al archivo:" << endl;
	      ...
	      if (archivo_lectura.is_open()) {
	         ...
	         if (archivo_escritura.is_open()) {
	            archivo_escritura<< cifrado(texto) << endl << contenido; // Escribir el texto al principio del archivo y luego el contenido leído anteriormente
...
avl *avl::update(avl *root)
{
	...
	if(strcmp(w,"SALIR")!=0){
		...
		gotoxy(50,17);
		std::cout <<"[1] Español [2] Inglés [3] Francés [4] Alemán [5] Italiano [0] Salir";
		gotoxy(50,18);
		cin>>idioma;
		...
		if(!encuentra){
				...
				cout<<"Su palabra no existe en el diccionario pero la encontraremos en linea..."<<endl;
		   	 	gotoxy(50,20);
		   	 	cout << "Ingrese el idioma original de la palabra [1] Español [2] Inglés [3] Alemán [4] Francés [5] Italiano";
		   	 	gotoxy(50,21);
		   	 	cin>>idioma_origen;
...
int buscarpalabra(){
	...
	//se usa para poder utilizar los caracteres como ñ, tildes, etc.
	setlocale(LC_ALL, ""); 
	...
	//Crea el menu y ciclo para traducir palabras
	    ...
	    while (idioma_valido == false){
		    ...
		    std::cout << "Ingrese el idioma al que desea traducir: "<<endl;
		    gotoxy(50,17);
			std::cout <<"[1] Español [2] Inglés [3] Alemán [4] Francés [5] Italiano [0] Salir";
		    gotoxy(50,18);
		    int palint=getch();
	    if ( palint==27) return 0;
	    ...
	    if(indice_idioma > 0) {
		    ...
		    for (int i = 0; i < palabras.size(); i++) {
				...
				//cout<<paword<<"-"<<endl;
				if (paword1 == palabra) {
					//cout<<"entro a buscar en español..."<<endl;
					//	system("pause");
		    ...
		    if (archivo.is_open()) {
		        ...
		        while (std::getline(archivo, linea)) {
		            ...
		            if(usuario==user){
						...
						if(!continua){
		            		gotoxy(50,17);
		            		cout<<"Usuario o contraseña incorrectos, intente de nuevo o cree un usuario";
		            		gotoxy(50,18);
		            		system("pause");
						}
					}
